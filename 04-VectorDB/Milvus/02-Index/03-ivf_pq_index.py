...
# 5. Create index (data is already in the collection at this point)
...
index_params.add_index(
    ...
    params={
        "nlist": 64,  # Number of cluster centroids, typically 4*sqrt(n), where n is the number of vectors
        "m": 32,      # Number of sub-vectors the vector is divided into, typically dim/m >= 2, here 128/32=4
        "nbits": 8    # Number of encoding bits per sub-vector, typically 8 bits
    }
)
client.create_index(
...
results = client.search(
    ...
    search_params={
        "params": {
            "nprobe": 10  # Set the number of clusters to check during search
        }
    }
)
