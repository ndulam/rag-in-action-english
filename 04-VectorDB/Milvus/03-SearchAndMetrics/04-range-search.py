...
print("\n=== Range Search ===")
# Using L2 distance metric, set range search parameters
# Note: for L2 distance, range_filter should be less than radius
results = client.search(
    collection_name=COLLECTION_NAME,
    data=[query_vector],
    anns_field="vector",
    limit=10,  # Increase limit to show more results
    search_params={
        "metric_type": "L2",
        "params": {
            "radius": 1.0,  # Outer circle radius
            "range_filter": 0.5  # Inner circle radius
        }
    },
    output_fields=["color"]
)
